...
public: //ham bo tro
	...
	void stable_sort(customer*& head, customer*& tail, int n) {
		...
		for (int i = 0; i < n; i++) {
			if (temp->next != tail) {
				if (temp->name[temp->name.size() - 1] > temp->next->name[temp->next->name.size() - 1]) {
					swap(temp, temp->next);
				}
			}
			else {
...
public:
	...
	void BLUE_OL() { //duoi all OAN LINH gan day nhat -> vao som nhat
		...
		while (tmp) {
			if (tmp->energy < 0) {
				cout << "OAN LINH trong queue bi moi ve: " << tmp->name << "|" << tmp->energy << endl;
				string n = tmp->name;
				tmp = tmp->prev;
				BLUE_queue(n);
			}
		...
		while (temp) {
			if (temp->energy < 0) {
				cout << "OAN LINH trong table bi moi ve la: " << temp->name << "|" << temp->energy;
				string n = temp->name;
				temp = temp->prev;
				BLUE(n);
			}
	...
	void BLUE_TS() {
		...
		while (tmp) {
			if (tmp->energy > 0) {
				cout << "THUAT SU trong queue bi moi ve: " << tmp->name << "|" << tmp->energy;
				string n = tmp->name;
				tmp = tmp->prev;
				BLUE_queue(n);
			}
			...
			for (int i = 0; i < qcount; i++) {
				...
				if (tcheck->next != nullptr) {
					if (tcheck->energy == tcheck->next->energy) {
						while (tcheck->energy == tcheck->next->energy) {
							tcheck = tcheck->next;
							n++;
						}
	...
	void REVERSAL()
	{
		...
		temp = x_cus;
		for (int i = 0; i < count; i++) {
			if (temp->energy == ener) x_cus = temp;
